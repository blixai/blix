    ...
    if (fs.existsSync(`./src/reducers/${globalReducer}.js`)) {
      ...
      if (body.indexOf(`case "${upperCase}":`) !== -1) {
        ...
        if (position !== -1) {
          ...
          console.log(
            "Reducer file found but something went wrong. Check the file for a switch statement."
          );
          rl.close();
        }
      }
          ...
          if (container.indexOf(mapDispatchToProps) !== -1) {
            ...
            fs.writeFile(
              ...
              err => {
                if (err) console.error(err);
                log(
                  `imported ${lowercase} into ${folder}Container and created mapDispatchToProps`
                );
              }
            );
          }
