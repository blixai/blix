        ...
        if (answer === 'y') {
          rl.question('Postgres or MongoDB? (P/M) ', (database) => {
            ...
            if (database === 'p') {
              ...
              installDevDependencies('babel-preset-env babelify gulp gulp-uglify gulp-rename browserify gulp-htmlmin gulp-clean-css gulp-tap gulp-buffer del run-sequence envify bundle-collapser')
              installKnexGlobal()
              modifyKnex()
              try {
                execSync(`createdb ${name};`, { stdio: 'ignore' });
