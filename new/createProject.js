...
let createReactRedux = () => {
  rl.question('Do you need a backend? (Y/N) ', (backend) => {
    ...
    if (backend === 'y') {
      ...
      installDevDependencies('webpack webpack-dev-server babel-loader css-loader babel-core babel-preset-env babel-preset-react style-loader webpack-merge uglifyjs-webpack-plugin sass-loader node-sass extract-text-webpack-plugin clean-webpack-plugin cssnano postcss postcss-cssnext postcss-import postcss-loader')
      process.stdout.write('\033c')
      log('The project was created!')
      log(`cd into ${name}`)
      log(`First start webpack: npm run build`)
