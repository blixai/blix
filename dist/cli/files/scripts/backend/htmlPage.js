"use strict";
var fs = require('fs');
var path = require('path');
var log = console.log;
var name = process.argv[2];
if (name !== undefined) {
    name = name.toLowerCase();
}
else {
    log('No page name supplied. Try npm run page <name>');
}
var pages = process.argv.splice(3, process.argv.length);
var html = fs.readFileSync(path.resolve(__dirname, './templates/htmlPageTemplate.html'), 'utf8');
html = html.replace(/Name/g, "" + name);
var css = "body {\n\tcolor: blue;\n}";
var js = "console.log('hello world')";
var route = "\nr.get('/" + name + "', (req, res) => res.sendFile(path.join(__dirname, '../dist/" + name + "/index.html')))";
var createPages = function () {
    pages.forEach(function (page) {
        page = page.toLowerCase();
        if (fs.existsSync("./src/" + name + "/" + page)) {
            log("The folder " + page + " already exists");
        }
        else {
            var pageRoute = "\nr.get('/" + name + "/" + page + "', (req, res) => res.sendFile(path.join(__dirname, '../dist/" + name + "/" + page + "/index.html')))";
            var subPage = fs.readFileSync(path.resolve(__dirname, './templates/htmlPageTemplate.html'), 'utf8');
            subPage = subPage.replace(/Name/g, "../" + page);
            fs.mkdirSync("./src/" + name + "/" + page);
            log("Created " + page + " folder in src/" + name + "/" + page);
            fs.writeFile("./src/" + name + "/" + page + "/index.html", subPage, function (err) {
                if (err)
                    throw err;
                log("Created page " + page + " src/" + name + "/" + page);
            });
            fs.appendFile('./server/routes.js', pageRoute, function (err) {
                if (err)
                    throw err;
                log("Added " + page + " route to routes.js");
            });
            fs.writeFile("./src/" + name + "/" + page + "/index.js", js, function (err) {
                if (err)
                    throw err;
                log("Created index.js file in src/" + name + "/" + page);
            });
            fs.writeFile("./src/" + name + "/" + page + "/main.css", css, function (err) {
                if (err)
                    throw err;
                log("Created main.css file in src/" + name + "/" + page);
            });
        }
    });
};
var newPage = function () {
    if (fs.existsSync("./src/" + name)) {
        createPages();
    }
    else {
        fs.mkdirSync("./src/" + name);
        fs.writeFile("./src/" + name + "/index.html", html, function (err) {
            if (err)
                throw err;
        });
        fs.writeFile("./src/" + name + "/main.css", css, function (err) {
            if (err)
                throw err;
        });
        fs.writeFile("./src/" + name + "/index.js", js, function (err) {
            if (err)
                throw err;
        });
        fs.appendFile("./server/routes.js", route, function (err) {
            if (err)
                throw err;
        });
        createPages();
    }
};
if (name) {
    newPage();
}
